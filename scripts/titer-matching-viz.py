...
YearStats = dict[str, dict[str, int]]  # {year: {"metadata_match": int, "potential_match": int, "total": int}}
...
def plot_matches(stats: list[Stats], output_file: str):
    ...
    for row_idx, center in enumerate(centers):
        for col_idx, data in enumerate(stats_per_center[row_idx]):
            ...
            percentages = {
                'strains': {x:data['strains'][x]['metadata_match']/data['strains'][x]['total']*100 for x in xvals if x in data['strains']},
                'strains_maybe': {x:data['strains'][x]['potential_match']/data['strains'][x]['total']*100 for x in xvals if x in data['strains']},
                'measurements': {x:data['measurements'][x]['metadata_match']/data['measurements'][x]['total']*100 for x in xvals if x in data['measurements']},
                'measurements_maybe': {x:data['measurements'][x]['potential_match']/data['measurements'][x]['total']*100 for x in xvals if x in data['measurements']},
            }
            ...
            strains_sizes = [data['strains'][x]['metadata_match']*circle_scalar if x in data['strains'] else 0 for x in xvals]
            strains_maybe_sizes = [data['strains'][x]['potential_match']*circle_scalar if x in data['strains'] else 0 for x in xvals]
            ...
            measurements_sizes = [data['measurements'][x]['metadata_match']*circle_scalar if x in data['measurements'] else 0 for x in xvals]
            measurements_maybe_sizes = [data['measurements'][x]['potential_match']*circle_scalar if x in data['measurements'] else 0 for x in xvals]
            ...
            for idx in [0,1]:
                ...
                keys = ['metadata_match', 'potential_match']
                pct_strains = total_strains[keys[idx]] / total_strains['total'] * 100
                pct_measurements = total_measurements[keys[idx]] / total_measurements['total'] * 100
                ...
                if idx==0:
                    ...
                    ax.text(0.02, 0.34, f'{total_strains["metadata_match"]:,} / {total_strains["total"]:,} ({pct_strains:.1f}%)', transform=ax.transAxes, color=c_strain, fontsize=12)
                    ax.text(0.02, 0.18, f'Unique measurements matched:', transform=ax.transAxes, color=c_measurement, fontsize=12)
                    ax.text(0.02, 0.10, f'{total_measurements["metadata_match"]:,} / {total_measurements["total"]:,} ({pct_measurements:.1f}%)', transform=ax.transAxes, color=c_measurement, fontsize=12)
                if idx==1:
                    ax.text(0.02, 0.26, f'maybes = {total_strains["potential_match"]:,} ({pct_strains:.1f}%)', transform=ax.transAxes, color=c_strain, fontsize=12)
                    ax.text(0.02, 0.02, f'maybes = {total_measurements["potential_match"]:,} ({pct_measurements:.1f}%)', transform=ax.transAxes, color=c_measurement, fontsize=12)
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("Visualisation script failed with error:", file=sys.stderr)
        print(e, file=sys.stderr)
        print("Exiting with code 0 so that automated pipelines don't die", file=sys.stderr)
