...
rule tip_frequencies:
    input:
        ...
        metadata = build_dir + "/{build_name}/metadata.tsv",
    params:
        narrow_bandwidth=config.get("frequencies", {}).get("narrow_bandwidth", 1 / 12.0),
        wide_bandwidth=config.get("frequencies", {}).get("wide_bandwidth", 3 / 12.0),
        proportion_wide=config.get("frequencies", {}).get("proportion_wide", 0.0),
        ...
        pivot_interval=config.get("frequencies", {}).get("pivot_interval", 1),
        pivot_interval_units=config.get("frequencies", {}).get("pivot_interval_units", "months"),
    output:
