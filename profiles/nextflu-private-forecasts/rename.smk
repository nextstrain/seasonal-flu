...
rule rename_measurements:
    input:
        lambda wildcards: _get_file_by_auspice_name(wildcards).replace(".json", "_measurements.json"),
    output:
        "auspice_renamed/{auspice_name}_measurements.json",
    shell:
        """
        ln {input} {output}
        """
