...
type MatchingStrainNames = defaultdict[Subtype, defaultdict[Contrib, set[str]]]
type TiterStrain = str
type NewStrainName = str
type Reason = str
type ComplexMatch = tuple[NewStrainName, Reason]
type MatchingStrainNamesComplex = defaultdict[Subtype, defaultdict[Contrib, defaultdict[TiterStrain, ComplexMatch] ]]
...
def read_titers(dir: str)-> TiterInfo:
    ...
    for subtype in SUBTYPES:
        for file in glob.glob(os.path.join(dir, subtype, '*.tsv.gz')):
            ...
            # WHO is a superset of all the other ones, so skip it
            if contributor == 'who':
                continue
...
def compute_simple_matches(name: str,
        ...
        titers: TiterInfo) -> tuple[TiterMatches, MatchingStrainNames]:
    ...
    for subtype in SUBTYPES:
        ...
        for contrib in titers[subtype]:
            for year, titer_strains in titers[subtype][contrib].items():
                ...
                matching_strains[subtype][contrib] |= matches
    return (data, matching_strains)
...
def compute_complex_matches(
        ...
        titers: TiterInfo) -> tuple[TiterMatches, MatchingStrainNamesComplex]:
    print(f"------ curated metadata vs titers (complex) matching --------", file=sys.stderr)
    data: TiterMatches = defaultdict(lambda: defaultdict(lambda: defaultdict()))
    log: MatchingStrainNamesComplex = defaultdict(lambda: defaultdict(lambda: defaultdict())) 
    ...
    for subtype in SUBTYPES:
        ...
        normalized_curated_strains = {simplify_strain(s): s for s in curated_metadata_strains}
            ...
            for year, titer_strains in titers[subtype][contrib].items():
                ...
                for titer_strain in titer_strains:
                    ...
                    if titer_strain in curated_metadata_strains:
                        num_matches+=1
                        log[subtype][contrib][titer_strain] = (titer_strain, 'simple_match')
                        continue
                    ...
                    if epi_isl:=fauna_strain_to_epi_isl.get(titer_strain, None):
                        if curated_strain:=epi_isl_to_curated_strain.get(epi_isl, None):
                            num_matches+=1
                            log[subtype][contrib][titer_strain] = (curated_strain, 'fauna_epi_isl_map')
                            continue
                    
                    # Normalize
                    if curated_strain:= normalized_curated_strains.get(simplify_strain(titer_strain)):
                        log[subtype][contrib][titer_strain] = (curated_strain, 'normalized_match')
                        num_matches+=1
                        continue
...
def log_matches(
        fname: str,
        titers: TiterInfo,
        fauna_matching_strains: MatchingStrainNames,
        curated_matching_strains_simple: MatchingStrainNames,
        curated_matching_strains_complex: MatchingStrainNamesComplex
        ):
    fh = open(fname, 'w')
    header = ('subtype', 'contributing_center', 'titer_strain', 'new_strain_name', 'fauna_match', 'curated_match_without_changes', 'curated_match_with_changes', 'change_type_required')
    print("\t".join(header), file=fh)

    # also print some STDOUT information
    print(f"{'subtype':<10}{'contrib':<8}{'year':<10}{'will_lose(simple_match)':<25}{'will_gain(simple_match)':<25}{'will_lose(after_remap)':<25}{'will_gain(after_remap)':<25}", file=sys.stderr)

    for subtype in titers.keys():
        [subtype_will_lose_simple, subtype_will_gain_simple,subtype_will_lose_remap, subtype_will_gain_remap] = [0, 0, 0, 0]
        for contrib in titers[subtype].keys():
            [contrib_will_lose_simple, contrib_will_gain_simple,contrib_will_lose_remap, contrib_will_gain_remap] = [0, 0, 0, 0]
            for year in sorted(titers[subtype][contrib].keys()):
                [year_will_lose_simple, year_will_gain_simple, year_will_lose_remap, year_will_gain_remap] = [0, 0, 0, 0]
                for titer_strain in titers[subtype][contrib][year]:
                    fauna_match = titer_strain in fauna_matching_strains[subtype][contrib]
                    curated_match_without_changes = titer_strain in curated_matching_strains_simple[subtype][contrib]

                    if fauna_match and curated_match_without_changes:
                        continue # don't print out a line if it's all happily matching

                    remap_match = curated_matching_strains_complex[subtype][contrib].get(titer_strain, False)

                    if remap_match:
                        remap_strain = remap_match[0] if remap_match[1]!='simple_match' else ''
                        remap_reason = remap_match[1]
                    else:
                        remap_strain = ''
                        remap_reason = ''

                    print("\t".join((subtype, contrib, titer_strain, remap_strain, str(fauna_match), str(curated_match_without_changes), str(bool(remap_match)), remap_reason)), file=fh)

                    
                    if fauna_match and not curated_match_without_changes:
                        subtype_will_lose_simple+=1
                        contrib_will_lose_simple+=1
                        year_will_lose_simple += 1
                    elif not fauna_match and curated_match_without_changes:
                        subtype_will_gain_simple+=1
                        contrib_will_gain_simple+=1
                        year_will_gain_simple += 1

                    if fauna_match and not remap_match:
                        subtype_will_lose_remap+=1
                        contrib_will_lose_remap+=1
                        year_will_lose_remap += 1
                    elif not fauna_match and remap_match:
                        subtype_will_gain_remap+=1
                        contrib_will_gain_remap+=1
                        year_will_gain_remap += 1
            
                if year_will_lose_simple or year_will_gain_simple:
                    print(f"{subtype:<10}{contrib:<8}{year:<10}{year_will_lose_simple:<25}{year_will_gain_simple:<25}{year_will_lose_remap:<25}{year_will_gain_remap:<25}", file=sys.stderr)
            print(f"{subtype:<10}{contrib:<8}{'ALL':<10}{contrib_will_lose_simple:<25}{contrib_will_gain_simple:<25}{contrib_will_lose_remap:<25}{contrib_will_gain_remap:<25}", file=sys.stderr)
        print(f"{subtype:<10}{'ALL':<8}{'ALL':<10}{subtype_will_lose_simple:<25}{subtype_will_gain_simple:<25}{subtype_will_lose_remap:<25}{subtype_will_gain_remap:<25}", file=sys.stderr)
    fh.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--curated", required=True, metavar='DIR',
                        help="Folder which is expected to have subfolders 'h3n2' etc each with a curated `metadata.tsv`")
    parser.add_argument("--output", default="fauna_matches.png",
                        help="Output file for the visualization (default: fauna_matches.png)")
    parser.add_argument("--log", help="Output TSV")

    args = parser.parse_args()

    titers = read_titers(args.titers)

    fauna = parse_metadata("fauna", args.fauna, "fauna-metadata.tsv")
    ...
    fauna_matches, fauna_matching_strains = compute_simple_matches("fauna", fauna_strains, titers)
    curated_matches, curated_matching_strains = compute_simple_matches("curated", curated_strains, titers)

    # complex matching 
    curated_matches_complex, curated_matches_complex_log = compute_complex_matches(curated, fauna, titers)

    plot_matches(fauna_matches, curated_matches, curated_matches_complex, args.output)

    log_matches(args.log, titers, fauna_matching_strains, curated_matching_strains, curated_matches_complex_log)
